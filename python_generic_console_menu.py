...
TIMER = True 			# True/False: whether or not a timer-msg shows under the menu at init. regarding DEFAULT_SELECTION: which runs automatically after time given ends
TIME_SEC = 33 			# works only if TIMER=True # time of msg for DEFAULT_SELECTION , defined in SECONDS , that when run-out DEFAULT_SELECTION is chosen automatically
SELECTION_MSG = True	# True/False: whether or not a msg prompt for up/down/enter keys is shown under the menu or not 

INCLUDE_QUIT = True 				# True/False, to add a "quit" item at the bottom of the menu, or not to add one
QUIT_TXT = "Exit Me Now !!!"		# works only if INCLUDE_QUIT=True # the text shown of the QUIT menu item !
...
WIDTH_FACTOR = 16  	# an integer between 0 and half (or better: less than half) of console width (in characters-count) 
...
COMPACT_ITEMS_LIST = False 	# True/False , if True: items list is compact, no spaces between items ... suitable for menus with many items!

LEFT_TABS = 3	# how many TABS the menu is spaced from the left side of the console window
TOP_LINES = 6	# how many lines the menu is spaced from the top of the console window
...
if not NO_BORDERS:
	...
	if ROUND_CORNERS and not DOUBLE_BORDERS:
		...
		TLC,TRC,BLC,BRC = "\u250c","\u2510","\u2514","\u2518"
	# define top,bottom,left,right borders from the building-blocks defined above  # for further details: https://en.wikipedia.org/wiki/Box-drawing_characters#Unicode
...
def show_auto_msg():
	...
	while counter>0 and not keypress:
		print(" * after ",counter," seconds , "+menu[DEFAULT_SELECTION]+" will run ...  ")
		time.sleep(1)
		counter -= 1
